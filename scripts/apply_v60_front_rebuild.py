...
# ---------------------------------------------------------------------------
# Clean v60 front replacement
# ---------------------------------------------------------------------------
# The old front/cage patch stack is deliberately not reused here.  The fixed
# front is rebuilt as one closed cross-carrier, analogous to the closed v60 side
# supports.  It contains only two smooth replaceable module bays.  Each module
# carries the spindle tunnel and the v50-style removable RH8x2 lead-nut
# cartridge; the fixed carrier itself contains no working lead thread.

SPINDLE_X = (-48.0, 128.0)
SPINDLE_SPACING = SPINDLE_X[1] - SPINDLE_X[0]
PLATE_X = 160.0
PLATE_CENTER_X = 40.0
PLATE_X0 = PLATE_CENTER_X - PLATE_X / 2.0
PLATE_X1 = PLATE_CENTER_X + PLATE_X / 2.0
PLATE_Y = F.PLATE_Y
PLATE_Z0 = F.PLATE_Z0
PLATE_Z1 = F.PLATE_Z1
PLATE_OPEN = F.PLATE_OPEN
PLATE_HOLE_D = F.PLATE_HOLE_D
...
# Replaceable cassette geometry.  The cassette drops in from +Z and is fixed by
# two M3 screws into heat-set inserts in the carrier.  The module can therefore
# be replaced without reprinting either long carrier.  Inside the cassette the
# v50 wear cartridge remains independently replaceable by cross-pin + C-clip.
MODULE_BODY_HALF_X = 12.0
MODULE_BAY_HALF_X = 12.35
MODULE_BODY_Y0 = FRONT_Y0 + 0.35
MODULE_BODY_Y1 = FRONT_Y1 - 0.35
MODULE_BODY_Z0 = 16.0
MODULE_BODY_Z1 = FRONT_Z1
MODULE_BAY_Z0 = 15.60
MODULE_FLANGE_HALF_X = 19.0
MODULE_FLANGE_Y0 = FRONT_Y0 + 1.0
MODULE_FLANGE_Y1 = FRONT_Y1 - 1.0
MODULE_FLANGE_Z0 = FRONT_Z1 - 0.20
...
def build_lead_nut():
    body = C.box(
        ...
        2.0 * LEAD_NUT_BODY_HALF_X, F.NUT_THREAD_LEN, 2.0 * LEAD_NUT_BODY_HALF_Z,
    )
    tail = C.box(
    ...
    C.require_single(q, 'new-front removable RH8x2 cartridge')
    return q
...
def build_nut_pin():
    ...
    ], 'new-front cartridge retaining pin')
    C.require_single(q, 'new-front cartridge retaining pin')
    return q
...
def build_module_local():
    ...
    q = q.cut(F.cyl_y(
        ...
        0.0, FRONT_Y0 - 1.0, F.SPINDLE_Z,
    )).removeSplitter()

    # v50-style cartridge cradle plus explicit +Y extraction mouth.  The body
    # channel and lower tail channel remain open to the module face so after
    # spindle, C-clip and cross-pin are removed the wear cartridge can slide out.
    body_channel = C.box(
        -8.35, F.NUT_THREAD_Y0 - 0.35,
    ...
    for hx in (-MODULE_M3_X_OFFSET, MODULE_M3_X_OFFSET):
        q = q.cut(Part.makeCylinder(
            ...
            App.Vector(hx, (MODULE_FLANGE_Y0 + MODULE_FLANGE_Y1) / 2.0,
                       MODULE_FLANGE_Z0 - 0.5),
            App.Vector(0, 0, 1),
        )).removeSplitter()
...
def build_plate():
    body_y0 = F.PLATE_SPINDLE_Y - PLATE_Y
    q = C.box(
        PLATE_X0, body_y0, PLATE_Z0,
    ...
    for sx in SPINDLE_X:
        ear = C.box(
            sx - PLATE_EAR_HALF_X, body_y0, PLATE_EAR_Z0,
            2.0 * PLATE_EAR_HALF_X, PLATE_Y, PLATE_EAR_Z1 - PLATE_EAR_Z0,
        )
        q = q.fuse(ear).removeSplitter()
        q = q.cut(F.cyl_y(
            PLATE_HOLE_D / 2.0, PLATE_Y + 1.0,
            sx, body_y0 - 0.5, F.SPINDLE_Z,
        )).removeSplitter()
        q = q.cut(F.cyl_y(
            6.0, 2.0, sx, body_y0, F.SPINDLE_Z,
        )).removeSplitter()
    C.require_single(q, 'new 160 mm clamp plate with outboard drive ears')
    return q


def rebuild_structural_core():
    ...
    ], 'new-front structural core without legacy crosshead')

    # Closed cross-carrier: same 30 mm structural height as the v60 side holms.
    front_beam = C.box(
        FRONT_X0, FRONT_Y0, FRONT_Z0,
        FRONT_X1 - FRONT_X0, FRONT_Y1 - FRONT_Y0, FRONT_Z1 - FRONT_Z0,
    )
    core = core.fuse(front_beam).removeSplitter()
    C.require_single(core, 'new-front core with closed cross carrier')

    # Re-open rack service paths that can be refilled by structural fusions.
    for xc in C.CLAMP_X:
        core = core.cut(C.cyl_x(C.UPPER_SADDLE_R, 40.0, xc - 20.0, 0.0, 0.0)).removeSplitter()
    ...
    # Two simple cassette bays plus four heat-set insert pockets per carrier.
    for sx in SPINDLE_X:
        bay = C.box(
        ...
        core = core.cut(bay).removeSplitter()
        for hx in (-MODULE_M3_X_OFFSET, MODULE_M3_X_OFFSET):
            core = core.cut(Part.makeCylinder(
                MODULE_M3_INSERT_D / 2.0, MODULE_M3_INSERT_DEPTH,
                App.Vector(sx + hx,
                           (MODULE_FLANGE_Y0 + MODULE_FLANGE_Y1) / 2.0,
                           FRONT_Z1 - MODULE_M3_INSERT_DEPTH),
                App.Vector(0, 0, 1),
            )).removeSplitter()

    C.require_single(core, 'new-front fixed carrier after replaceable module bays')
    return core


stage('build clean closed front carrier and two replaceable clamp modules')
_t = start_timer('box_front_rebuild.build_geometry')
RIGHT_FULL = rebuild_structural_core()
...
stage('hard-check modular service architecture and clamp motion')
...
if SPINDLE_SPACING != 176.0:
    failures.append(f'unexpected spindle spacing {SPINDLE_SPACING}')
if abs(PLATE.BoundBox.XMin - (SPINDLE_X[0] - PLATE_EAR_HALF_X)) > 1e-6:
    failures.append('left plate drive ear is not at module axis')
if abs(PLATE.BoundBox.XMax - (SPINDLE_X[1] + PLATE_EAR_HALF_X)) > 1e-6:
    failures.append('right plate drive ear is not at module axis')

# Fixed carrier must have a clean smooth bay at both module positions.
for sx in SPINDLE_X:
    mod = MODULE.copy(); mod.translate(App.Vector(sx, 0, 0))
    common = RIGHT_FULL.common(mod).Volume
    module_checks.append({'x_mm': sx, 'base_common_mm3': round(common, 6)})
    if common > 1e-4:
        failures.append(f'replaceable module collides with fixed carrier X={sx}: {common:.6f}')
    ...
    if nmc > 1e-4 or pmc > 1e-4 or cmc > 1e-4:
        failures.append(f'v50 service hardware blocked by replaceable module X={sx}')

# Explicit cartridge extraction witness through the +Y service mouth.
extraction = []
for dy in (0.0, 2.0, 5.0, 10.0, 16.0):
    nut = LEAD_NUT.copy(); nut.translate(App.Vector(SPINDLE_X[0], F.NUT_Y0 + dy, F.SPINDLE_Z))
    mod = MODULE.copy(); mod.translate(App.Vector(SPINDLE_X[0], 0, 0))
    cv = mod.common(nut).Volume
    extraction.append({'travel_y_mm': dy, 'module_common_mm3': round(cv, 6)})
    if cv > 1e-4:
        failures.append(f'cartridge cannot extract through module service mouth dy={dy}: {cv:.6f}')

# Moving plate remains outside the fixed front and modules over full stroke.
...
# RH8x2 motion uses the same proven spindle/female master pair as v50.
for sx in SPINDLE_X:
    nut = LEAD_NUT.copy(); nut.translate(App.Vector(sx, F.NUT_Y0, F.SPINDLE_Z))
    mod = MODULE.copy(); mod.translate(App.Vector(sx, 0, 0))
    for travel in (-0.5, 0.0, 0.5, 1.0, 2.0, 3.0, 4.0, 5.5):
        ...
        if bc > 1.0 or mc > 1.0 or nc > 1.0:
            failures.append(
                f'RH8x2 motion collision X={sx} travel={travel}: '
                f'base={bc:.6f} module={mc:.6f} nut={nc:.6f}'
            )

# Phase sensitivity witness: axial half-pitch without rotation must interfere.
...
C.export_shape('eurobox_v60_lead_nut', LEAD_NUT)
C.export_shape('eurobox_v60_lead_nut_pin', NUT_PIN)
C.export_shape('eurobox_v60_lead_nut_clip', NUT_PIN_CLIP)
...
validation['box_clamp'] = {
    ...
    'effective_total_width_mm': validation.get('box_clamp', {}).get('effective_total_width_mm', 600.0),
