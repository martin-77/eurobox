...
# ---------------------------------------------------------------------------
# Clean-front v3: keep the validated v2 carrier/plate datums, but fix two
# mechanical defects in the replaceable clamp cassette:
#   1) the spindle drive/hex sat behind cassette material;
#   2) the lead-nut retaining tail met the threaded body through a tiny butt
#      neck and would snap at that transition.
#
# The fixed carrier already has a full rectangular cassette bay, so neither
# correction needs any change to the validated carrier, rack datums or plate.
# ---------------------------------------------------------------------------
...
# Keep the v2 top flange and bay fit, but start the structural cassette body
# only just behind the working RH8x2 section.  The complete 10 AF drive hex is
# therefore in an open rear service window instead of buried behind a wall.
...
# Strong cartridge tail.  v2 had a 12 x 5.5 x 5.5 mm tail ending exactly at
# the threaded body's rear face and overlapping it vertically by only 0.5 mm.
# v3 makes the tail full body width and overlaps the body by 6 mm in Y and
# 3.5 mm in Z.  The retaining pin sits inside that reinforced overlap region.
...
def stage(msg):
    print('V60_FRONT_REBUILD_V3 ' + msg, flush=True)
...
def build_lead_nut():
    ...
    C.require_single(q, 'clean-front-v3 reinforced removable RH8x2 cartridge')
    return q
...
def build_module_local():
    ...
    # The rear part of the cassette bay remains completely open.  Only the
    # working thread/cartridge region is enclosed by the cassette body.
    q = q.cut(F.cyl_y(
    ...
    # Full-width cartridge body channel.
    q = q.cut(C.box(
        -8.35,
    ...
    # Reinforced tail channel.  It starts behind MODULE_BODY_Y0, so the tail and
    # pin are serviceable from the same rear opening as the spindle drive.
    q = q.cut(C.box(
        -8.35,
        NUT_TAIL_Y0 - 0.35,
    ...
    C.require_single(q, 'clean-front-v3 rear-open replaceable clamp cassette')
    return q
...
stage('build rear-open cassette and reinforced RH8x2 cartridge')
_t = start_timer('box_front_rebuild_v3.build_geometry')
MODULE = build_module_local()
LEAD_NUT = build_lead_nut()
NUT_PIN = build_nut_pin()
NUT_PIN_CLIP = V2.NUT_PIN_CLIP
stop_timer('box_front_rebuild_v3.build_geometry', _t)

stage('hard-check drive access, cartridge strength and RH8x2 motion')
_t = start_timer('box_front_rebuild_v3.hard_checks')
failures = []
module_checks = []
thread_motion = []

# The drive hex is the segment immediately behind the RH8x2 working thread.
# At -0.5 mm preload the whole spindle shifts +0.5 mm in Y, which is its most
# demanding position relative to the rear-open cassette body.
...
# Prove a real open service corridor through both fixed carrier and cassette.
# The probe encloses the 10 AF hex with >0.4 mm radial clearance and extends
# from behind the fixed beam to the new cassette-body start plane.
service_probe_local = C.box(
    -6.20,
    V2.FRONT_Y0 - 2.0,
    F.SPINDLE_Z - 6.20,
    12.40,
    MODULE_BODY_Y0 - (V2.FRONT_Y0 - 2.0),
    12.40,
)
for sx in SPINDLE_X:
    probe = service_probe_local.copy()
    probe.translate(App.Vector(sx, 0, 0))
...
# Structural bridge witness: this exact 16 x 6 x 2.5 mm region did not exist in
# v2.  It is deliberately below the RH8x2 major radius and above the retaining
# pin, so it must remain almost completely solid after all functional cuts.
bridge_probe = C.box(
    -8.0,
    -F.NUT_THREAD_LEN,
    -7.0,
...
if bridge_fraction < 0.995:
    failures.append(
        f'lead-nut reinforced body/tail bridge is not materially solid: '
        f'{bridge_fraction:.6f}'
    )

# Installed cartridge/pin/clip fit.
for sx in SPINDLE_X:
    mod = MODULE.copy()
    mod.translate(App.Vector(sx, 0, 0))
...
# Preserve the proven RH8x2 kinematics and phase; only the non-threaded support
# geometry changed.
for sx in SPINDLE_X:
    nut = LEAD_NUT.copy()
    nut.translate(App.Vector(sx, F.NUT_Y0, F.SPINDLE_Z))
...
stop_timer('box_front_rebuild_v3.hard_checks', _t, failures=len(failures))
if failures:
    raise RuntimeError('V60 CLEAN FRONT V3 HARD CHECKS FAILED: ' + ' | '.join(failures))

stage('re-export corrected service parts')
_t = start_timer('box_front_rebuild_v3.exports')
for name, shape in {
...
stop_timer('box_front_rebuild_v3.exports', _t)
...
box['architecture'] = 'clean_modular_front_v3_rear_open_drive_and_reinforced_RH8x2_cartridge'
...
box['lead_nut_body_tail_bridge_witness_mm'] = [16.0, 6.0, 2.5]
...
validation['stage'] = 'full_direct_mechanism_clean_modular_front_v3'
validation['failures'] = []
